...
public class UrlHandler {
	...
	public boolean shouldOverrideUrlLoading(WebView view, String url) {
		...
		if (rlzProviderPresent()) {
			...
			if (needsRlzString(siteUri)) {
				...
				// AsyncTask. Although we are not overriding the URL load
				// synchronously,
				// we guarantee that we will handle this URL load after the task
				// executes,
				// so it's safe to just return true to WebCore now to stop its
				// own loading.
	...
	boolean handleMenuClick(String url) {
	...
	// Taken largely from Chrome source,
	// src/chrome/browser/google_url_tracker.cc
	private static boolean needsRlzString(Uri uri) {
		...
		if (("http".equals(scheme) || "https".equals(scheme))
			...
			if (googleComponent > 0
					&& "corp".equals(hostComponents[googleComponent - 1])) {
				return false;
			}
